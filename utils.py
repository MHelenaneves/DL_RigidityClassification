...
def plot_accu_metric(hist, file_name, metric='accuracy'):
    plt.figure()
    plt.plot(hist.history[metric])
    plt.plot(hist.history['val_' + metric])
    plt.title('model ' + metric)
...
def save_logs_t_leNet(output_directory, hist, y_pred, y_true, duration, y_test):
    ...
    #plot accuracy
    plot_accu_metric(hist, output_directory + 'epochs_accu.png')
    #plot confusion matrix
    #plot_confusion(y_test,y_pred, output_directory + 'Confusion Matrix.png')


def save_logs(output_directory, hist, y_pred, y_true, y_test, duration,lr=True, y_pred_val=None, y_true_val=None):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)
